...
func NewSubmittedTask[T any, R any](task T, id int64, future *Future[R, int64]) *SubmittedTask[T, R] {
	return &SubmittedTask[T, R]{
		Task:   task,
		Id:     id,
		Future: future,
	}
...
func (f *Future[R, K]) AddResult(r Result[R, K]) {
	f.result <- r
}
